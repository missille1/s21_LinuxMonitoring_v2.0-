...
purge_roots_from_stdin() {
	_roots_only | sort -u | while read -r d; do
		[ -n "$d" ] || continue
		rm_base_dir "$d"
	done
}
...
delete_by_log() {
	log="$1"
	echo "[лог] $log"
	awk -F'|' '$1=="DIR"{print $2}' "$log" | purge_roots_from_stdin
...
delete_by_time() {
	...
	for base in $(pick_bases); do
		find "$base" -xdev -type d -name '*_[0-9][0-9][0-9][0-9][0-9][0-9]' \
			-newermt "$t_start" ! -newermt "$t_end_plus" -print 2>/dev/null
	done | purge_roots_from_stdin
