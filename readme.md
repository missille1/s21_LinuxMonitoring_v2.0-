...
Написал bash-скрипт. Скрипт запускается с 6 параметрами. Пример запуска скрипта: \
...
Написал bash-скрипт. Скрипт запускается с 3 параметрами. Пример запуска скрипта: \
...
В конце работы скрипта выводится на экран время начала работы скрипта, время окончания и общее время его работы. Дополни этими данными лог-файл.


## Part 3. Очистка файловой системы

Написал bash-скрипт. Скрипт запускается с 1 параметром.
...
Написал bash-скрипт или программу на С, генерирующие 5 файлов логов **nginx** в *combined* формате.
...
В комментариях к своему скрипту/программе указал, что означает каждый из использованных кодов ответа.


## Part 5. Мониторинг

Написал bash-скрипт для разбора логов **nginx** из [Части 4] через **awk**.
...
### Отдельный install.md в ./src/07/install.md по конфигурированию 

##### Установил и настроил **Prometheus** и **Grafana** на виртуальную машину.
##### Получил доступ к веб-интерфейсам **Prometheus** и **Grafana** с локальной машины.
...
##### Запустил свой bash-скрипт из [Части 2].
##### Посмотри на нагрузку жесткого диска (место на диске и операции чтения/записи).

##### Установил утилиту **stress** и запусти команду `stress -c 2 -i 1 -m 1 --vm-bytes 32M -t 10s`
##### Посмотрел на нагрузку жесткого диска, оперативной памяти и ЦПУ.


## Part 8. Готовый дашборд

### Отдельный install.md в ./src/08/install.md по конфигурированию 

##### Установил готовый дашборд *Node Exporter Quickstart and Dashboard* с официального сайта **Grafana Labs**.

##### Провел те же тесты, что и в [Части 7].
...
##### Посмотрел на нагрузку сетевого интерфейса.


## Part 9. Дополнительно. Свой *node_exporter*

Написал bash-скрипт, собирающие информацию по базовым метрикам системы (ЦПУ, оперативная память, жесткий диск (объем)).
Скрипт или программа формирует html страничку по формату **Prometheus**, которую будет отдавать **nginx**. \
Сама страничка обновляется внутри bash-скрипта раз в 3 секунды.

##### Поменял конфигурационный файл **Prometheus**, чтобы он собирал информацию с созданной мной странички.
